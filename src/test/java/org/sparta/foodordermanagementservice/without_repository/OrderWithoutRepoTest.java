...
class OrderWithoutRepoTest {
    ...
    void paginateOrders() throws Exception {
        ...
        mockMvc.perform(get("/api/orders")
                ...
                .andExpect(jsonPath("$.data.content[0].status").value(result0.getStatus().name()))
                .andExpect(jsonPath("$.data.content[0].type").value(result0.getType().name()))
...
//        UUID targetOrderId = UUID.randomUUID(); // 실패케이스: 존재하지 않는 주문 아이디
        ...
        mockMvc.perform(get("/api/orders/{id}", targetOrderId))
                ...
                .andExpect(jsonPath("$.data.orderStatus").value(order.getStatus().name()))
                .andExpect(jsonPath("$.data.orderType").value(order.getType().name()))
